...
# проверка перехода в Булки, Соусы, Начинки через нажание на текст
def test_click_text_bread_sauce_topping(logged_in_account):
    logged_in_account.find_element(By.XPATH, ".//div/span[text()='Соусы']").click()
    assert logged_in_account.find_element(By.XPATH, ".//main//h2[2]").text == "Соусы"

# Параметризированный тест
@pytest.mark.parametrize("text", ["Соусы", "Начинки"])
def test_click_text_bread_sauce_topping(logged_in_account, text):
